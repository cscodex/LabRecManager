...
interface QuestionBankPickerProps {
    ...
    excludeIds?: string[]; // IDs to exclude (already in exam)
    onCancel: () => void;
}

export default function QuestionBankPicker({ onImport, excludeIds = [], onCancel }: QuestionBankPickerProps) {
    ...
    const loadQuestions = useCallback(async (isLoadMore = false) => {
        ...
        try {
            const params = new URLSearchParams({
                ...
                search,
                tagId: selectedTag,
                excludeParegraphSubQuestions: 'true' // Only fetch parent questions
            });
            ...
            if (data.success) {
                // Filter out already excluded IDs locally if needed, though API does pagination
    ...
    return (
        <div className="flex flex-col h-[70vh]">
            ...
            <div className="flex-1 overflow-y-auto border rounded-xl bg-gray-50 p-2 space-y-2">
                {questions.filter(q => !excludeIds.includes(q.id)).map(q => (
                    <div
                        key={q.id}
                        onClick={() => toggleSelection(q.id)}
                        className={`p-3 bg-white rounded-lg border cursor-pointer border-l-4 hover:shadow-sm transition-all ${selectedIds.has(q.id) ? 'border-l-blue-600 ring-1 ring-blue-100' : 'border-l-transparent'}`}
                    >
