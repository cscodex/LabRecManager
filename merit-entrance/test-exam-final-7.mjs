...
async function run() {
    ...
    try {
        const adminRes = await pool.query("SELECT id, email FROM \"User\" WHERE role = 'admin' LIMIT 1");
        if (adminRes.rows.length === 0) throw new Error("No admin");
        const admin = adminRes.rows[0];
        console.log(`Using Admin: ${admin.email} / ${admin.id}`);
        
        const bpRes = await pool.query("SELECT id, name FROM \"ExamBlueprint\" LIMIT 1");
        ...
        if (matReq.rows.length > 0) {
            ...
            console.log(`Executing Vector Query: SELECT chunk_text FROM "DocumentChunk" ${contextFilter} LIMIT 2`);
